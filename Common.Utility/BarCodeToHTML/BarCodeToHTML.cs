...
namespace Common.Utility
{
    public class BarCodeToHTML
    {
        ...
        public static string get39(string s, int width, int height)
        {
            ...
            var result_html = ""; //HTML代码
            var color = ""; //颜色
            foreach (var c in result_bin)
            {
            ...
            foreach (var c in s)
                result_html += "<div style=\"width:" + width * (len + 1) +
        ...
        public static string getEAN13(string s, int width, int height)
        {
            ...
            {
                if (!Regex.IsMatch(s, @"^\d{13}$")) return "存在不允许的字符！";

                checkcode_input = int.Parse(s[12].ToString());
                s = s.Substring(0, 12);
            }

            var sum_even = 0; //偶数位之和
            var sum_odd = 0; //奇数位之和
            ...
            var checkcode = (10 - (sum_even * 3 + sum_odd) % 10) % 10; //校验码

            if (checkcode_input > 0 && checkcode_input != checkcode) return "输入的校验码错误！";
            ...
            for (var i = 1; i < 7; i++) result_bin += ean13(s[i], type[i - 1]);
            result_bin += "01010";
            for (var i = 7; i < 13; i++) result_bin += ean13(s[i], 'C');
            result_bin += "101000000000";

            var result_html = ""; //HTML代码
            var color = ""; //颜色
            var height_bottom = width * 5;
            foreach (var c in result_bin)
            {
                color = c == '0' ? "#FFFFFF" : "#000000";
                result_html += "<div style=\"width:" + width + "px;height:" + height + "px;float:left;background:" +
            ...
            result_html += "<div style=\"float:left;color:#000000;width:" + width * 9 + "px;text-align:center;\">" +
                           s[0] + "</div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom +
            ...
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom +
                           "px;background:#000000;\"></div>";
            for (var i = 1; i < 7; i++)
                result_html += "<div style=\"float:left;width:" + width * 7 + "px;color:#000000;text-align:center;\">" +
                               s[i] + "</div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom +
                           "px;background:#FFFFFF;\"></div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom +
