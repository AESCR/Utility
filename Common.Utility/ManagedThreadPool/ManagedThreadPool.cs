...
{
    /// <summary>
    /// 用C#托管代码实现的线程池 它在管理线程的时候，有一个缓存线程的池，即一个ArrayList对象
    ...
    public class ManagedThreadPool
    {
        ...
        /// <summary>
        /// Used to signal that a worker thread is needed for processing. Note that multiple threads
        /// may be needed simultaneously and as such we use a semaphore instead of an auto reset event.
        /// </summary>
        ...
        {
            // Create our thread stores; we handle synchronization ourself as we may run into
            // situtations where multiple operations need to be atomic. We keep track of the threads
            // we've created just for good measure; not actually needed for any core functionality.
        ...
        /// </summary>
        /// <param name="callback">
        /// A WaitCallback representing the delegate to invoke when the thread in the thread pool
        /// picks up the work item.
        /// </param>
        ...
        {
            // Create a waiting callback that contains the delegate and its state. At it to the
            // processing queue, and signal that data is waiting.
        ...
        private static void ProcessQueuedItems()
        {
            ...
            while (true)
            {
                ...
                // Get the next item in the queue. If there is nothing there, go to sleep for a
                // while until we're woken up when a callback is waiting.
                WaitingCallback callback = null;

                // Try to get the next callback available. We need to lock on the queue in order to
                // make our count check and retrieval atomic.
                lock (_poolLock)
                {
                ...
                if (callback != null)
                    // We now have a callback. Execute it. Make sure to accurately record how many
                    // callbacks are currently executing.
                    try
                    {
    ...
    public class Semaphore
    {
        ...
        public void P()
        {
            // Lock so we can work in peace. This works because lock is actually built around Monitor.
            lock (_semLock)
            {
                // Wait until a unit becomes available. We need to wait in a loop in case someone
                // else wakes up before us. This could happen if the Monitor.Pulse statements were
                // changed to Monitor.PulseAll statements in order to introduce some randomness into
                // the order in which threads are woken.
        ...
        public void V()
        {
            ...
            lock (_semLock)
            {
                // Release our hold on the unit of control. Then tell everyone waiting on this
                // object that there is a unit available.
                _count++;
