    ...
    public class XMLProcess
    {
        ...
        public static bool DeleteXmlRowByIndex(string strXmlPath, int iDeleteRow)
        {
            try
            {
                var ds = new DataSet();
                ds.ReadXml(GetXmlFullPath(strXmlPath));
                if (ds.Tables[0].Rows.Count > 0) ds.Tables[0].Rows[iDeleteRow].Delete();
                ds.WriteXml(GetXmlFullPath(strXmlPath));
                return true;
            }
            catch (Exception)
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                    for (var i = 0; i < ds.Tables[0].Rows.Count; i++)
                        ...
                        {
                            //循环给找到行的各列赋新值
                            for (var j = 0; j < Columns.Length; j++) ds.Tables[0].Rows[i][Columns[j]] = ColumnValue[j];
        ...
        public DataView GetDataViewByXml(string strWhere, string strSort)
        {
            try
            {
                ...
                if (strSort != null) dv.Sort = strSort; //对DataView中的记录进行排序
                if (strWhere != null) dv.RowFilter = strWhere; //对DataView中的记录进行筛选，找到我们想要的记录
                return dv;
            }
            catch (Exception)
            {
