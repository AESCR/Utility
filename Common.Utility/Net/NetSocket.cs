...
namespace Common.Utility.Net
{
    ...
    public class NetSocket
    {
        ...
        {
            if (!socket.IsBound) socket.Bind(endPoint);
        }
        ...
        public static int GetValidPort(string port)
        {
            ...
            {
                //传入的端口号为空则抛出异常
                if (port == "") throw new Exception("端口号不能为空！");

                //检测端口范围
                if (Convert.ToInt32(port) < MINPORT || Convert.ToInt32(port) > MAXPORT) throw new Exception("端口号范围无效！");
        ...
        {
            //创建本地终结点
            var localPoint = CreateIPEndPoint(LocalHostName, port);
